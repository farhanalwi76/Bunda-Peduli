...
<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="flex items-center justify-end mt-4">
            @if (Route::has('password.request'))
                <a class="underline text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 light:focus:ring-offset-gray-800" href="{{ route('password.request') }}">
    ...
    <div class="mt-4 text-center">
        <p class="text-sm text-gray-600 dark:text-gray-400">
            {{ __("Belum punya akun?") }} 
            <a href="{{ route('register') }}" class="text-indigo-600 hover:text-indigo-900 dark:hover:text-indigo-300">
                {{ __('Daftar di sini') }}
            </a>
        </p>
    </div>
