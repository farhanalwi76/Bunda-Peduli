    ...
    @if($rombongan_belajar)
        <form class="forms-sample" action="{{ url('dashboard/rombel/update', $rombongan_belajar->id) }}" method="POST">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-8">
                    <input type="text" class="form-control" id="kode" name="kode" value="{{ $rombongan_belajar->kode }}" placeholder="Masukkan kode Lengkap">
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-sm-8">
                    <select class="form-control" name="dosen_pa" id="dosen_pa">
                        @foreach ($dosen as $dsn)
                            <option value="{{ $dsn->nidn }}" {{ $dsn->nidn == $rombongan_belajar->dosen_pa ? 'selected' : '' }}>
                                {{ $dsn->nama }}
                            </option>
                        @endforeach
                    </select>
